...
The number of occupants per room within a house, however, is largely 1 or less across all counties. A few counties, e.g., Monterey, have a higher percentage of housing units where the occupants per room is greater than 1, signaling a more populated housing environment.
...
df_selected <- df %>% 
  ...
  drop_na() %>% 
  mutate(race = if_else(str_detect(race, "non-white"), "Largely non-white", "Largely white")
  )
...
* Percentage of white demograhpics within the county population has a negative correlation with the COVID death rate. This may indicate largely white counties are more likely to be better equipped to deal with COVID infection.
...
df_selected %>% 
  ...
  mutate(race = if_else(str_detect(race, "non-white"), "Largely non-white (n = 18)", "Largely white (n = 40)")) %>% 
  ggplot(aes(x = race, y = covid_deaths / 100)) + 
  geom_jitter(width = 0.1) +
  stat_summary(fun.y=median, geom="point", shape=18,
                 size=3, color="red") +
  scale_y_continuous(labels = scales::percent) +
  # labs(title="Counties with largely non-whte population have a higher COVID death rate", subtitle="(Median values highlighted in red)", x="Dominant Population Within County", y = "COVID Death Rate per 100K People") +
...
df_selected %>% 
  ...
  stat_summary(fun.y=median, geom="point", shape=18,
                 size=3, color="red") +
  scale_y_continuous(labels = scales::percent) +
  labs(title="Counties with largely Hispanic or Latino population have a higher COVID death rate", subtitle="(Median values highlighted in red)", x="Dominant Population Within County", y = "COVID Death Rate per 100K People") +
  theme_minimal()
