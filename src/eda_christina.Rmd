...
* Percentage of white demograhpics within the county population has a negative correlation with the COVID death rate. This may indicate dominantly white counties are more likely to be better equipped to deal with COVID infection.
...
```{r corr_plot}
corr <- round(cor(df_selected_renamed %>% select(is.numeric) %>% rename_with(~ str_replace_all(.x, "_", " ") %>% str_to_title())), 1)
corr

# Compute a matrix of correlation p-values
p.mat <- cor_pmat(df_selected_renamed %>% select(is.numeric))
p.mat
...
df_selected %>% 
  ...
  stat_summary(fun.y=median, geom="point", shape=18,
                 size=3, color="red") +
  scale_y_continuous(labels = scales::percent) +
  labs(title="Counties with dominantly non-whte population have a higher COVID death rate", subtitle="(Median values highlighted in red)", x="Dominant Population Within County", y = "COVID Death Rate per 100K People") +
  theme_minimal()
...
df_selected %>% 
  ...
  stat_summary(fun.y=median, geom="point", shape=18,
                 size=3, color="red") +
  scale_y_continuous(labels = scales::percent) +
  labs(title="Counties with dominantly Hispanic or Latino population have a higher COVID death rate", subtitle="(Median values highlighted in red)", x="Dominant Population Within County", y = "COVID Death Rate per 100K People") +
  theme_minimal()
...
df_lm <- df_selected %>% 
  select(race, pp_household, parks_usage, covid_deaths)

lm.fit = lm(covid_deaths ~ race + pp_household + parks_usage, data = df_lm)
summary(lm.fit)

lm.fit = lm(covid_deaths ~ race + parks_usage, data = df_lm)
summary(lm.fit)
